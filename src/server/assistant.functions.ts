...
// =====================================================================
// Anomaly explanations: AI-generated "why" + "what to do next" per issue
// =====================================================================
...
export interface AnomalyExplanation {
  ...
  hotelId: string;
  hotelName: string;
  title: string; // short headline restated by the AI
  why: string; // 1-2 sentence plain-language root-cause hypothesis
  actions: string[]; // 2-4 short imperative next steps
  severity: "critical" | "warning" | "info";
  // Confidence the AI has in its hypothesis given the available data.
  ...
  anomalies: z
    .array(
      z.object({
        ...
        utility: z.string().max(40).optional(),
...
export const explainAnomalies = createServerFn({ method: "POST" })
  .inputValidator(anomalyInputSchema)
  .handler(
    async ({
      data,
    }): Promise<
      | { ok: true; explanations: AnomalyExplanation[] }
      | { ok: false; error: string }
    > => {
      if (data.anomalies.length === 0) {
        return { ok: true as const, explanations: [] };
      }

      const { context } = await getPortfolioContext();

      // Compact, model-friendly bullet list of every anomaly to explain.
      const anomaliesBlock = data.anomalies
        .map((a, i) => {
          const period =
            a.year && a.month
              ...
              : "";
          const change =
            a.pctChange !== undefined
              ? ` (${a.pctChange >= 0 ? "+" : ""}${a.pctChange.toFixed(0)}%)`
              : "";
          return `${i + 1}. id=${a.id} | hotel="${a.hotelName}" | kind=${a.kind} | severity=${a.severity}${
            period ? ` | period=${period}` : ""
          }${a.utility ? ` | utility=${a.utility}` : ""}${change} — ${a.title}${
            a.detail ? ` (${a.detail})` : ""
          }`;
        })
        .join("\n");
      ...
      const messages: ChatMsg[] = [
        {
          role: "system",
          content: `You are Sera, a hotel sustainability consultant. For each anomaly below, give a manager a plain-language explanation of WHY it likely happened and 2-4 SHORT next-step actions they can take today. Ground every hypothesis in the hotel's actual data — never invent numbers. If the data is too thin to be sure, say so and lower confidence.
  ...
  "explanations": [
    {
      ...
      "title": "<= 10 word restatement of the issue, no numbers>",
      "why": "1-2 short sentences (<=40 words total) explaining the most likely root cause, citing the hotel name and period",
      "actions": ["2-4 imperative actions, each <= 14 words, starting with a verb (Check, Compare, Re-read, Ask, Replace, Review)"],
      "confidence": "low" | "medium" | "high"
    }
  ]
}
Tone: warm, direct, never alarmist. Avoid jargon. Use units (kWh, m³, kg, %).
Order the explanations to MATCH the input order. Return one explanation per input anomaly.

ANOMALIES TO EXPLAIN (${data.anomalies.length}):
${anomaliesBlock}
...
${context}`,
        ...
        {
          role: "user",
          content: "Explain these anomalies and tell me what to do next.",
        },
      ];
      ...
      try {
        ...
        console.error("explainAnomalies AI call failed:", e);
        return { ok: false as const, error: "Insights unavailable right now." };
      }
      ...
      if (!res.ok) {
        console.error("explainAnomalies error:", res.status);
        return { ok: false as const, error: "Insights unavailable right now." };
      }
      ...
      try {
        ...
        const byId = new Map(parsed.explanations.map((e) => [e.id, e]));

        // Hydrate against the original anomalies so we always get the
        // hotel/severity even if the model omits them, and we keep order.
        const explanations: AnomalyExplanation[] = data.anomalies.map((a) => {
          ...
          return {
            ...
            severity: a.severity,
            title: String(e?.title ?? a.title).slice(0, 120),
            why: String(e?.why ?? "").slice(0, 360),
            actions: (e?.actions ?? [])
              .slice(0, 4)
        ...
        console.error("Failed to parse anomaly explanations:", err);
        return { ok: false as const, error: "Could not parse insights." };
      }
    },
  );
